...
\begin{itemize}
	\item Fixed probe set of $n=256$ input pairs, drawn once, containing both
		train and test pairs, frozen across all runs and seeds. Report metrics on
		train and test halves separately as well as pooled.
	\item eNTK via \texttt{torch.func} (\texttt{jacrev} + \texttt{vmap}),
		sum-over-logits kernel (gradient of $\sum_c f_c$), $256\times256$ per
		checkpoint, seconds to compute. First-logit variant as a robustness check.
	\item All of $S,R,A$ computed on the centred kernel $\Kt=HKH$; uncentred $A$
		also reported for comparability with \citet{kumar2024grokking}.
...
\TODO{Subject to change (particularly tier 0 alpha sweep differs to match kumar paper)}
...
\TODO{Note that we use accuracy rather than loss for grokking detection}
...
\begin{itemize}
	...
	\item Robustness: uncentred alignment, first-logit eNTK, $\tau$ sensitivity,
		train/test halves of the probe set.
\end{itemize}

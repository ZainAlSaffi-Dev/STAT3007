...
\emph{Grokking} is a phenomenon where the training loss of a neural network
...
$\tau_{\mathrm{test}}$. Grokking has been observed in networks trained on modular addition and, as such, it will be the task. The input concatenates the one-hot codes of $a,b\in\{0,\dots,p-1\}$ and the target is the one-hot code of $(a+b)\bmod p$. The neural tangent kernel (NTK)
of a network $f(x;\theta)$ is
...
\citep{jacot2018ntk,chizat2019lazy}. One account of grokking is a lazy-to-rich transition, where a linearised model memorises but generalising needs
the kernel to move \citep{kumar2024grokking, mohamadi2024grok}.
...
the eigenbasis fixed, or it can change in \emph{shape}, so that the eigenbasis itself
changes (e.g. rotation). Let $\Kh=K/\Fnorm{K}$ be the normalised kernel, and define the scale
term $S_t=\log(\Fnorm{K_t}/\Fnorm{K_0})$ and the shape term
...
In our terms $S_t=-2(k-1)\lambda t$ and $R_t\equiv0$, so the kernel shrinks but
never changes shape and no features are learned. Yet
...
they call speculative and do not test on the kernel.
% Rotation is also
% necessary: a permutation-equivariant model that stays in the kernel regime provably needs a constant
...
shape, kernel or weight scale, and the weight-decay rate.
Three 2026 accounts each back a different one (\S2), and none measures
shape change. We measure all three on one grid and ask which $\tgrok$ tracks.

Our hypothesis H is that shape change is the cause. Let $Y$ be the probe set's
one-hot label matrix and $H=I-\tfrac1n\mathbf{1}\mathbf{1}^\top$ the centring
matrix. The cosine $A_t$ between $HK_tH$ and $HYY^\top H$ is centred
alignment \citep{cortes2012alignment}. Rescaling $K_t$ leaves it
unchanged, so only shape change moves it. H says generalisation begins when $A_t$ crosses a
...
shape change is a finite-width effect and a wider network changes shape more slowly. Under
H the target $A_*$ is fixed, so $\tgrok$ is a distance over a rate and should
...
and $b\approx-1$. Two nonzero exponents mean two clocks, and the slower one sets the time.
...
$\tgrok\propto(1-\beta)/(\eta\lambda)$ where $\beta$ controls heavy-ball momentum. All three are scale-like or
optimiser-level quantities, so none can tell a reshaped kernel from a decaying
one. H supplies the missing variable $A_t$. Our baseline uses MSE and vanilla
...
that separates width-driven reshaping from the $\lambda$-clock, and a shape-based
account of the norm law under MSE. If the timing of feature learning is set by kernel shape, then a quantity that is measurable during the apparently-flat phase of training predicts capability onset before any loss-based metric does, which provides an early-warning signal for delayed generalisation.

\section{Feasibility}

The baseline follows \citet[App.~8.3, arXiv v3]{kumar2024grokking}, so
...
on $n=256$ probe points, summed over logits, takes seconds to run.
...
$\eta=\eta_0/\alpha^2$ \citep{chizat2019lazy, kumar2024grokking}. Since $\alpha$, initialisation
...
$\alpha\times\eta\lambda$ at the baseline width. One is a $4\times4$ grid in
$N\times\eta\lambda$, with $N$ from $50$ to $1600$ and $\eta\lambda$ from $0$
to $10^{-1}$, for the $(a,b)$ fit. One is a clamp arm with five values of
$\rho$ between $1$ and $2$, and one is a generality arm over three primes $p$
and three training fractions. Five seeds per cell, and ten on the clamp arm
where the MSE effect is half-size, give about $350$ runs and roughly $20$
GPU-hours.
...
% The work is intended to be divided as follows: Zac Kienzle builds the infrastructure, Aaron D'Mello
